...
func executeOnlySnapshot(config *structs.Config, snapshotConfig *structs.SnapshotConfig) error {
	...
	if err == nil {
		...
		stringOutput := string(cpOutput)
		if len(stringOutput) > 10000 {
			stringOutput = stringOutput[:10000]
		}
		if cpErr != nil {
			return fmt.Errorf("%s error copying last snapshot %s to %s: %s, %s", snapshotLogPrefix, newestSnapshotPath, tmpDir, cpErr.Error(), stringOutput)
		}
	...
	for _, dirToSnapshot := range snapshotConfig.Dirs {
		...
		stringOutput := string(rsyncOutput)
		if len(stringOutput) > 10000 {
			stringOutput = stringOutput[:10000]
		}
		if err != nil {
			return fmt.Errorf("%s can't sync %s/ to %s: %s, %s", snapshotLogPrefix, dirToSnapshot.SrcDirAbspath, dstDirFull, err.Error(), stringOutput)
		}
	}
	...
	if len(snapshotConfig.PreSnapshotCommands) > 0 {
		...
		for _, command := range snapshotConfig.PreSnapshotCommands {
			...
			if len(result) > 0 {
				...
				slog.Info(snapshotLogPrefix + " " + command + ": " + stringOutput)
			}
		}
		after := time.Now().UnixMilli()
		seconds := float64(after-before) / 1000
...
func GetSnapshotsInfo(snapshotsConfigsDir string, expandVars bool, snapshotName string) (snapshotsInfo []*structs.SnapshotInfo, err error) {
	...
	fmt.Println("Getting snapshot infos of " + snapshotName + " which has snapshots dir " + snapshotConfig.SnapshotsDir)
	for _, entry := range snapshotsDirsEntries {
		snapshotFullPath := path.Join(snapshotConfig.SnapshotsDir, entry.Name())
		fmt.Println("snapshot full path is " + snapshotFullPath)
...
func RestoreSnapshot(config *structs.Config, number int, snapshotConfig *structs.SnapshotConfig) (err error) {
	...
	for _, dir := range snapshotConfig.Dirs {
		...
		if err != nil {
			...
			slog.Error(fmt.Sprintf("%s can't sync %s/ to %s: %s, %s", snapshotLogPrefix, snapshottedDirPath, dir.SrcDirAbspath, err.Error(), stringOutput))
		}
	}
	return err
}
...
func RunInitCommands(config *structs.Config) error {
	for _, command := range config.InitCommands {
		...
		if err != nil {
			...
			return fmt.Errorf("can't execute command %s: %s, %s", command, err.Error(), stringOutput)
		}
	}
	return nil
}
